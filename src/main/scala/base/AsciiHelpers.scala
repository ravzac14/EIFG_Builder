...
object AsciiHelpers {
  ...
  private def isAsciiCharWhitespace(ac: Seq[String]): Boolean =
    !isAsciiCharEmpty(ac) && ac.forall(_.trim.isEmpty)

  def buildAsciiString(title: String): String =
    buildAsciiCharArray(title).mkString("\n")
